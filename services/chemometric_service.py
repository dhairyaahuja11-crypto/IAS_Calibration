...
class ChemometricAnalyzer:
    ...
    def perform_pls(self, X: np.ndarray, y: np.ndarray,
        ...
        y_pred_cv = cross_val_predict(self.pls_model, X, y, cv=cv).ravel()
    ...
    def get_pls_summary(self) -> str:
        ...
        fold_r2_mean = np.mean([f['r2'] for f in self.pls_results['fold_metrics']])
        fold_r2_std = np.std([f['r2'] for f in self.pls_results['fold_metrics']])
