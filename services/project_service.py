...
class ProjectService:
    _project_has_instrument_column = None
    ...
    @staticmethod
    def _ensure_project_instrument_column(conn=None):
        """Ensure the project table has an instrument column for project-level persistence."""
        if ProjectService._project_has_instrument_column is not None:
            return ProjectService._project_has_instrument_column

        owns_connection = conn is None
        local_conn = conn or ProjectService._get_db_connection()
        if not local_conn:
            ProjectService._project_has_instrument_column = False
            return False

        cursor = local_conn.cursor()
        try:
            cursor.execute("SHOW COLUMNS FROM project LIKE 'instrument'")
            ...
            if not has_column:
                ...
                local_conn.commit()
                has_column = True
            ProjectService._project_has_instrument_column = has_column
        except Exception:
            ProjectService._project_has_instrument_column = False
        finally:
            cursor.close()
    ...
    @staticmethod
    def _flatten_selected_sample_ids(selected_samples):
        """Expand grouped sample selections into unique linked sample IDs."""
        all_sample_ids = []
        for sample in selected_samples or []:
            if isinstance(sample, dict):
                sample_ids = sample.get('sample_ids')
                if sample_ids:
                    all_sample_ids.extend(str(sample_id).strip() for sample_id in sample_ids if str(sample_id).strip())
                    continue
        ...
        return list(dict.fromkeys(all_sample_ids))
    ...
    @staticmethod
    def _instrument_summary_sql(project_alias="project"):
        """Build a correlated SQL fragment that summarizes linked sample instruments."""
        return f"""
            COALESCE((
                SELECT GROUP_CONCAT(DISTINCT NULLIF(TRIM(md.device_id), '') ORDER BY md.device_id SEPARATOR ', ')
                FROM project_sample ps_instr
                INNER JOIN model_data md ON ps_instr.sample_id = md.sample_id
                WHERE ps_instr.project_id = {project_alias}.project_id
            ), '')
        """

    @staticmethod
    def _project_instrument_select_sql(project_alias="project", include_column=False):
        """Select project-level instrument when available, else fall back to linked sample summary."""
        instrument_summary = ProjectService._instrument_summary_sql(project_alias)
        if include_column:
            return (
                f"COALESCE(NULLIF(TRIM({project_alias}.instrument), ''), "
                f"NULLIF(TRIM({instrument_summary}), ''), '')"
            )
    ...
    def get_projects_by_filters(date_from, date_to, status=None, measurement_type=None,
        ...
        try:
            ...
            include_project_instrument = ProjectService._ensure_project_instrument_column(conn)
            query = """
                SELECT 
                    project_id,
                    project_name,
                    sample_type,
                    {instrument_summary} as instrument,
            ...
            """.format(
                instrument_summary=ProjectService._project_instrument_select_sql(
                    "project",
                    include_column=include_project_instrument
                )
            )
            ...
            if instrument and instrument.strip() and instrument.lower() != 'all':
                query += """
                    AND (
                        COALESCE(NULLIF(TRIM(project.instrument), ''), '') = %s
                        OR EXISTS (
                            SELECT 1
                            FROM project_sample ps_filter
                            INNER JOIN model_data md_filter ON ps_filter.sample_id = md_filter.sample_id
                            WHERE ps_filter.project_id = project.project_id
                            AND md_filter.device_id = %s
                        )
                    )
                """
                params.extend([instrument.strip(), instrument.strip()])
                ...
                query = f"""
                    SELECT
                        ...
                        COALESCE((
                            SELECT GROUP_CONCAT(DISTINCT NULLIF(TRIM(md.device_id), '') ORDER BY md.device_id SEPARATOR ', ')
                            FROM model_data md
                            WHERE md.sample_id = s.sample_id
                        ), '') as instrument
                    FROM sample s
                ...
                query = """
                    SELECT 
                        ...
                        COALESCE((
                            SELECT GROUP_CONCAT(DISTINCT NULLIF(TRIM(md.device_id), '') ORDER BY md.device_id SEPARATOR ', ')
                            FROM model_data md
                            WHERE md.sample_id = sample.sample_id
                        ), '') as instrument
                    FROM sample
    ...
    def update_project(project_id, project_data, selected_samples=None):
        ...
        try:
            ...
            instrument = '' if instrument is None else str(instrument).strip()[:255]
            ...
            if include_project_instrument:
                update_fields.append("instrument = %s")
                update_params.append(instrument)
            ...
            if selected_samples is not None:
                ...
                normalized_sample_ids = ProjectService._flatten_selected_sample_ids(selected_samples)
    ...
    def create_project(project_data, selected_samples):
        ...
        try:
            ...
            try:
                ...
                if include_project_instrument:
                    insert_columns.insert(3, "instrument")
                    insert_values.insert(3, instrument)
                ...
                if has_linking_table:
                    ...
                    all_sample_ids = ProjectService._flatten_selected_sample_ids(selected_samples)
