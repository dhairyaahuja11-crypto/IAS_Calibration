...
class SampleService:
    ...
    def get_samples_by_date(date_from, date_to, sample_name=None, user_id=None, sample_status=None):
        """
        INQUIRY BUTTON
        Fetch one row per sample based on creation date range and optional filters.
        Scan rows remain in model_data, but sample management shows only sample-level records.
        ...
        try:
            ...
            query = """
                SELECT 
                    ...
                    COUNT(md.model_id) as scanned_number,
                    CASE 
                    ...
                    COALESCE(
                        GROUP_CONCAT(DISTINCT NULLIF(TRIM(md.device_id), '') ORDER BY md.device_id SEPARATOR ', '),
                        ''
                    ) as instrument,
                    s.create_person as user_id,
                    DATE_FORMAT(MAX(md.create_time), '%%Y-%%m-%%d %%H:%%i:%%s') as creation_time,
                    s.sample_id as sample_id
                FROM sample s
                LEFT JOIN model_data md ON md.sample_id = s.sample_id
                ...
                WHERE (
                    DATE(COALESCE(md.create_time, s.create_time)) BETWEEN %s AND %s
                )
            ...
            query += """
                GROUP BY
                    s.sample_id, s.sample_name, s.model_num, s.model_wavemin, s.model_wavemax,
                    s.model_wavepath, s.model_method, s.sample_status, s.create_person,
                    s.property_value1, s.property_value2, s.property_value3, s.property_value4,
                    s.property_value5, s.property_value6, s.property_value7, s.property_value8,
                    s.property_value9, s.property_value10,
                    cd1.content_name, cd2.content_name, cd3.content_name, cd4.content_name, cd5.content_name,
                    cd6.content_name, cd7.content_name, cd8.content_name, cd9.content_name, cd10.content_name
                ORDER BY MAX(COALESCE(md.create_time, s.create_time)) DESC
            """
    ...
    def update_sample(sample_id, sample_data):
        ...
        try:
            ...
            cursor.execute(
                "SELECT sample_name FROM sample WHERE sample_id = %s AND (sample_state IS NULL OR sample_state != 'Deleted')",
                (sample_id,)
            )
            ...
            target_sample_name = sample_row.get('sample_name', '')
            
            # Get content_dictionary mapping (property name -> id)
            cursor.execute("SELECT id, content_name FROM content_dictionary")
            content_rows = cursor.fetchall()
            content_dict = {row['content_name'].lower(): row['id'] for row in content_rows}
            reverse_dict = {row['id']: row['content_name'].lower() for row in content_rows}
            ...
            cursor.execute("""
                ...
                FROM sample
                WHERE sample_name = %s AND (sample_state IS NULL OR sample_state != 'Deleted')
                ORDER BY sample_id
                LIMIT 1
            """, (target_sample_name,))
            ...
            query = f"""
                UPDATE sample SET
                    {', '.join(set_clauses)}
                WHERE sample_name = %s
                  AND (sample_state IS NULL OR sample_state != 'Deleted')
            """
            
            # Build values tuple
            updated_sample_name = sample_data.get('sample_name', '').strip() or target_sample_name
            base_values = [
                updated_sample_name,
            ...
            values = tuple(base_values + property_values + [target_sample_name])
            ...
            if rows_affected > 0:
                return True, "Sample updated successfully for all scans of this sample"
            else:
                return False, "No sample found with the given ID"
    ...
    def batch_import_substance_content(file_path, selected_sample_ids=None):
        ...
        try:
            ...
            for idx, row in df.iterrows():
                try:
                    # Get sample ID (sample-level ID from UI) and sample name for matching
                    sample_id = None
                    sample_name = None
