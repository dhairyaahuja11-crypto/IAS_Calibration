...
class SampleSelectionDialog(QDialog):
    ...
    def populate_table(self, samples):
        ...
        for sample in samples:
            ...
            if instrument is None:
                instrument = ''
            checkbox.setProperty("instrument", str(instrument).strip())
    ...
    def get_selected_samples(self):
        """Get one selected record per visible sample group."""
        selected = []
        for row in range(self.sample_table.rowCount()):
            ...
            if checkbox_widget:
                ...
                if checkbox and checkbox.isChecked():
                    ...
                    sample_ids = [str(sample_id).strip() for sample_id in sample_ids if str(sample_id).strip()]
                    sample_data = {
                        'sample_id': sample_ids[0] if sample_ids else checkbox.property("sample_id"),
                        ...
                        'instrument': checkbox.property("instrument") or ''
                    }
